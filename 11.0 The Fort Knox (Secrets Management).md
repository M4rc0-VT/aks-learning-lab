...
# **Day 11: The Fort Knox (Secrets Management)** 🔐

### **Summary**

We are going to stop managing secrets inside Kubernetes. Instead, we will use **Azure Key Vault (AKV)**—the industry standard for storing sensitive data in Azure. We will configure your cluster to "lease" secrets from the vault only when it needs them, using the **Secrets Store CSI Driver**.
...
### **Step 2: Terraform - Building the Vault** 🏗️
...
If a hacker steals your `etcd` database, they get nothing. If they steal your Git repo, they get nothing. This is "Bank Grade" security. 🏦
...
### **2. Infrastructure Changes (Terraform)** 🏗️
...
### **3. The Troubleshooting Log ("War Stories")** ⚔️
...
```powershell
az aks show --resource-group rg-terraform-lab --name aks-terraform-lab --query "addonProfiles.azureKeyvaultSecretsProvider.identity.clientId"
